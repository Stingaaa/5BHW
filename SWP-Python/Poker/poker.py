...
def generateStatistic():
    s = {"Royal Flush":0, "Straight Flush":0, "Four of a kind":0, "Full House":0, "Flush":0, "Straight":0, "Two Pair":0, "Pair":0, "Highest Number":0}
    return s
def generateCards():
    cards = []
...
def checkHand(cardsUsed, showCards):
                ...
                if(int(splitCard(cardsToNumbers[i])[0]) == int(splitCard(cardsToNumbers[j])[0]) == int(splitCard(cardsToNumbers[k])[0])):
                    ...
                    for i in range(len(cardsUsed)):
                        for j in range(i+1, len(cardsUsed)):
                            if(int(splitCard(cardsToNumbers[i])[0]) == int(splitCard(cardsToNumbers[j])[0]) and not int(splitCard(cardsToNumbers[i])[0]) == threeOfAKind):
    ...
    for i in range(0,3):
        for j in range(1+i,4):
            for k in range(1+j,5):
                if(int(splitCard(cardsToNumbers[i])[0]) == int(splitCard(cardsToNumbers[j])[0]) == int(splitCard(cardsToNumbers[k])[0])):
                    return "There are three " + splitCard(cardsToNumbers[i])[0] + "s in your hand!"
